...
async function recordVisit(config, req) {
    ...
    const wasNewToday = await redisCommand(config, ['SET', dailyKey, '1', 'EX', '172800', 'NX']);

    if (wasNewToday === 'OK') {
        await redisCommand(config, ['INCR', 'visits:total']);
    }
...
module.exports = async function handler(req, res) {
    ...
    if (!config) {
        return res.status(503).json({ configured: false, total: null });
    }
    ...
    try {
        ...
        return res.status(500).json({ configured: true, total: null, error: 'Failed to update visit count' });
    }
};
